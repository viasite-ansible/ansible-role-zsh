...
sudo ansible-playbook -i "localhost," -c local /tmp/zsh.yml --extra-vars="dotfile_repo_url=DOT_FILE_REPO_URL python3_command=python3 pip3_command=pip3"

title "Provision playbook for $(whoami)"
sudo ansible-playbook -i "localhost," -c local /tmp/zsh.yml --extra-vars="zsh_user=$(whoami) dotfile_repo_url=DOT_FILE_REPO_URL python3_command=python3 pip3_command=pip3"
